...
class ExternalAdapterLoader:
    ...
    def get_adapter(config, model):
        ...
        """
        if not hasattr(config.model, 'external_adapter') or not config.model.external_adapter:
        ...
        if not script_path or not class_name:
            raise ValueError("External adapter config must include script_path and class_name")
