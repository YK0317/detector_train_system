...
class CapsuleForensicsTrainer:
    ...
    def __init__(self, config, model, train_loader, val_loader, device, output_dir):
        ...
        if self.train_loader is None:
            raise ValueError("Train loader is None. Please check your dataset paths in the configuration.")
        if self.val_loader is None:
            raise ValueError("Validation loader is None. Please check your dataset paths in the configuration.")
    ...
    def _setup_capsule_models(self):
        ...
        try:
            ...
            config_gpu_id = getattr(self.config.model, 'model_args', {}).get('gpu_id', 0)
            
            # Determine actual GPU ID based on device availability
            if self.device.type == 'cuda' and torch.cuda.is_available() and config_gpu_id >= 0:
                gpu_id = config_gpu_id
                print(f"   🔥 Using GPU {gpu_id}")
            else:
                gpu_id = -1  # Use -1 to indicate CPU mode
                if config_gpu_id >= 0:
                    print(f"   ⚠️  GPU {config_gpu_id} requested but not available, falling back to CPU")
                else:
            ...
            beta1 = getattr(self.config.training, 'optimizer_params', {}).get('beta1', 0.9)
    ...
    def _load_checkpoint(self, epoch: int):
        ...
        try:
            ...
            if optim_path.exists():
                ...
                print(f"   ✅ Loaded checkpoint from epoch {epoch} (optimizer state not found)")
    ...
    def train(self) -> Dict[str, Any]:
        ...
        try:
            ...
            random_routing = not getattr(self.config.model, 'model_args', {}).get('disable_random', False)
    ...
    def _train_epoch(self, epoch: int, dropout: float, random_routing: bool) -> float:
        ...
        with tqdm(self.train_loader, desc=f"Training") as pbar:
            for i, (images, labels) in enumerate(pbar):
                ...
                classes, class_probs = self.capnet(vgg_features, random_routing, dropout)
    ...
    def _save_best_checkpoint(self, epoch: int):
        ...
        shutil.copy2(
            self.output_dir / f'capsule_{epoch}.pt',
            self.output_dir / 'capsule_best.pt'
        )
...
if __name__ == "__main__":
    ...
    print("   - CapsuleForensicsTrainer: Original Capsule-Forensics-v2 training methodology")
