...
class UnifiedDataset(Dataset):
    ...
    def _load_folder_data(self, split: str):
        ...
        if split == 'train':
            ...
            self._use_automatic_split = (self.config.val_path is None and 
                                       getattr(self.config, 'auto_split_enabled', True))
        elif split == 'val':
            # Handle case where val_path is None - use train_path for automatic splitting
            if self.config.val_path is None:
                data_path = Path(self.config.train_path)
                self._use_automatic_split = getattr(self.config, 'auto_split_enabled', True)
            else:
        ...
        if hasattr(self, '_use_automatic_split') and self._use_automatic_split:
            ...
            if auto_split_enabled:
                ...
                if hasattr(self.config, 'split_seed') and self.config.split_seed is not None:
                ...
                if split == 'train':
                    ...
                    logging.info(f"Auto-split enabled: {train_split_ratio:.1%} train, {1-train_split_ratio:.1%} validation")
                    logging.info(f"Training split: {len(train_samples)} samples from {len(all_samples)} total")
                elif split == 'val':
                    self.samples = val_samples
                    logging.info(f"Validation split: {len(val_samples)} samples from {len(all_samples)} total")
            else:
                # Use all samples (no split)
                self.samples = all_samples
                if split == 'val':
                    logging.info("Auto-split disabled: using all training data for validation")
        else:
