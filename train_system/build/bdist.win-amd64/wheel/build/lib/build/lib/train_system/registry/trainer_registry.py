...
class TrainerRegistry:
    ...
    def scan_for_trainers(self, additional_paths: List[str] = None):
        ...
        logger.info(f"Found {len(self._trainers)} trainers: {list(self._trainers.keys())}")
    ...
    def _is_trainer_class(self, cls) -> bool:
        ...
        if any(skip in module_name for skip in ['torch.', 'numpy.', 'builtins', 'ultralytics']):
            return False
        ...
        if not all(hasattr(cls, method) and callable(getattr(cls, method)) 
        ...
        try:
            ...
            common_params = {'config', 'model', 'train_loader', 'val_loader', 
                           'optimizer', 'criterion', 'device', 'logger'}
            param_matches = sum(1 for param in init_params 
    ...
    def register_trainer(self, name: str, trainer_class: Type, force: bool = False):
        ...
        if name in self._trainers and not force:
            logger.warning(f"Trainer '{name}' already registered. Use force=True to override.")
            return
