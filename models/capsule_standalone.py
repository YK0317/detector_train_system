...
class RoutingLayer(nn.Module):
    ...
    def forward(self, x, random=False, dropout=0.0):
        ...
        if random:
            noise = torch.Tensor(0.01*torch.randn(*self.route_weights.size())).to(self.route_weights.device)
        ...
        if dropout > 0.0:
            drop = torch.Tensor(torch.FloatTensor(*priors.size()).bernoulli(1.0- dropout)).to(priors.device)
            priors = priors * drop
...
def main():
    ...
    parser.add_argument('--model', type=str, default='trained/capsule_best.pth',
