...
class BLIPDeepfakeDetector(nn.Module):
    ...
    def forward(self, x: torch.Tensor, targets: Optional[torch.Tensor] = None, return_features: bool = False):
        """
        Forward pass
    ...
    def load_pretrained_weights(self, checkpoint_path: str, strict: bool = False):
        ...
        for key, value in state_dict.items():
            ...
            elif 'patch_embed' in key or 'pos_embed' in key or 'cls_token' in key or 'blocks' in key or 'norm' in key:
                # Direct ViT weights
                visual_encoder_dict[f'visual_encoder.{key}'] = value
        
        # Load weights
        missing_keys, unexpected_keys = self.load_state_dict(visual_encoder_dict, strict=False)
...
def create_blip_deepfake_detector(vit_arch: str = 'base', 
                                  img_size: int = 384,
                                  pretrained: bool = True,
                                  **kwargs) -> BLIPDeepfakeDetector:
    """
...
if __name__ == "__main__":
    # Create model
    model = create_blip_deepfake_detector(vit_arch='base', img_size=384, pretrained=False)
