...
class UnifiedDataset(Dataset):
    ...
    def _load_folder_data(self, split: str):
        ...
        if split == "train":
            ...
            # Check if we need to handle auto-split for training
            self._use_automatic_split = self.config.val_path is None and getattr(
                self.config, "auto_split_enabled", True
            )
        elif split == "val":
            # Handle case where val_path is None - use train_path for automatic splitting
            if self.config.val_path is None:
                data_path = Path(self.config.train_path)
                self._use_automatic_split = getattr(
                    self.config, "auto_split_enabled", True
                )
            else:
                data_path = Path(self.config.val_path)
                self._use_automatic_split = False
        ...
        # Handle automatic splitting
        if hasattr(self, "_use_automatic_split") and self._use_automatic_split:
            # Get split configuration
            auto_split_enabled = getattr(self.config, "auto_split_enabled", True)
            train_split_ratio = getattr(self.config, "train_split_ratio", 0.8)

            if auto_split_enabled:
                # Perform automatic train/val split
                import random

                # Set seed for reproducible splits if provided
                if (
                    hasattr(self.config, "split_seed")
                    and self.config.split_seed is not None
                ):
                ...
                for class_idx, class_samples in samples_by_class.items():
                    class_train_size = int(len(class_samples) * train_split_ratio)
                    train_samples.extend(class_samples[:class_train_size])
                    val_samples.extend(class_samples[class_train_size:])

                # Use appropriate samples for this split
                if split == "train":
                    self.samples = train_samples
                    logging.info(
                        f"Auto-split enabled: {train_split_ratio:.1%} train, {1-train_split_ratio:.1%} validation"
                    )
                    logging.info(
                        f"Training split: {len(train_samples)} samples from {len(all_samples)} total"
                    )
                elif split == "val":
                    self.samples = val_samples
                    logging.info(
                        f"Validation split: {len(val_samples)} samples from {len(all_samples)} total"
                    )
            else:
                ...
                if split == "val":
                    logging.info(
                        "Auto-split disabled: using all training data for validation"
                    )
        else:
            # No automatic splitting - use all samples
            self.samples = all_samples
    ...
    def _load_class_folders_data(self, split: str):
        ...
        if split == "train":
            ...
            if (
                hasattr(self.config, "train_path")
                and self.config.train_path
                and hasattr(self.config, "real_path")
                and self.config.real_path
                and hasattr(self.config, "fake_path")
                and self.config.fake_path
            ):
            ...
            # Option 2: Use absolute real_path and fake_path (backward compatibility)
                ...
                hasattr(self.config, "real_path")
                and self.config.real_path
                and hasattr(self.config, "fake_path")
                and self.config.fake_path
                and (
                    Path(self.config.real_path).is_absolute()
                    or Path(self.config.fake_path).is_absolute()
                )
            ):
            ...
            if not class_paths:
                raise ValueError(
                    "No class folders specified. Use either:\n"
            ...
            if (
                ...
                if hasattr(self.config, "val_path") and self.config.val_path:
                    ...
                    if val_base.exists():
                        ...
                        logging.warning(
                            "No validation path found, using training paths"
                        )
                        base_path = Path(self.config.train_path)
                        class_paths["real"] = str(base_path / self.config.real_path)
                        class_paths["fake"] = str(base_path / self.config.fake_path)
                ...
                if val_base and val_base.exists():
                    ...
                    if val_base.exists():
                        for (
                            class_name,
                            folder_name,
                        ) in self.config.class_folders.items():
                            class_paths[class_name] = str(val_base / folder_name)
                    else:
                        logging.warning(
                            "No validation folder found, validation split will be empty"
                        )
            ...
            if hasattr(self.config, "test_path") and self.config.test_path:
                ...
                if (
                    ...
                    class_paths = {
                        class_name: str(test_base / class_name)
    ...
    def _load_csv_data(self, split: str):
        ...
        if df["label"].dtype == "object":
            ...
            if (
                not hasattr(self.config, "class_mapping")
                or not self.config.class_mapping
            ):
                self.config.class_mapping = {
                    label: idx for label, idx in label_to_idx.items()
                }
        else:
        ...
        for class_idx, count in class_counts.items():
            if hasattr(self.config, "class_mapping") and self.config.class_mapping:
                class_name = next(
                    (
                        name
                        for name, idx in self.config.class_mapping.items()
                        if idx == class_idx
                    ),
                    str(class_idx),
                )
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            dummy_image = Image.new(
                "RGB", (self.config.img_size, self.config.img_size), color="black"
            )
            if self.transform:
...
class DatasetFactory:
    ...
    @staticmethod
    def from_csv(csv_path: str, img_size: int = 224, **kwargs) -> UnifiedDataset:
        """
        ...
        config = DataConfig(
            type="csv", train_path=str(csv_path), img_size=img_size, **kwargs
        )

        return UnifiedDataset(config, split="train")

    @staticmethod
    def from_json(json_path: str, img_size: int = 224, **kwargs) -> UnifiedDataset:
        """
        ...
        config = DataConfig(
            type="json", train_path=str(json_path), img_size=img_size, **kwargs
        )
