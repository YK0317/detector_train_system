    ...
    def load_external_trainer(script_path: str, class_name: str = None) -> type:
        ...
        if not script_path.exists():
            raise FileNotFoundError(
                f"External training script not found: {script_path}"
            )
        ...
        if class_name:
            ...
            for name in dir(module):
                ...
                if (
                    ...
                    and ("trainer" in name.lower() or "training" in name.lower())
                ):
                    trainer_class = obj
                    break
    ...
    def _validate_trainer_interface(trainer_class: type):
        ...
        for param in required_params:
            if param not in init_sig.parameters:
                print(
                    f"⚠️  Warning: External trainer __init__ missing parameter: {param}"
                )
        ...
        if len(train_sig.parameters) < 1:  # self parameter
            raise ValueError(
                "External trainer 'train' method should accept self parameter"
            )
...
class HybridTrainer:
    ...
    def _load_external_trainer(self):
            ...
            try:
                ...
                if TrainerClass:
                    ...
                    if "ultralytics" in module_name:
                        print(
                            f"⚠️  Detected ultralytics YOLO class, which should not be used as trainer"
                        )
                        print(
                            f"   Please use 'yolobuiltin' instead of 'yolo' for proper YOLO training"
                        )
                        # Fallback to the YOLOBuiltinTrainer
                        try:
                            from ..registry import get_component_by_name

                            TrainerClass = get_component_by_name(
                                "trainer", "yolobuiltin"
                            )
                            if not TrainerClass:
                                raise ValueError(
                                    f"YOLOBuiltinTrainer not found in registry"
                                )
                    ...
                    try:
                        ...
                        if "unexpected keyword argument" in str(e):
                            print(
                                f"⚠️  Trainer {TrainerClass.__name__} doesn't follow standard interface"
                            )
                            print(f"   Error: {e}")
                            raise ValueError(
                                f"Trainer '{name}' has incompatible constructor signature: {e}"
                            )
                        else:
                            raise

                    print(
                        f"✅ External trainer loaded from registry: {TrainerClass.__name__}"
                    )
                    return
                else:
                    print(
                        f"⚠️  Trainer '{name}' not found in registry, falling back to script_path"
                    )
            except ImportError:
        ...
        if not script_path:
            if name:
                raise ValueError(
                    f"External trainer '{name}' not found in registry and no script_path specified"
                )
            else:
                raise ValueError(
                    "External trainer enabled but no name or script_path specified"
                )

        print(f"🔄 Loading external trainer from: {script_path}")

        # Load trainer class
        TrainerClass = ExternalTrainerLoader.load_external_trainer(
