...
class ModelFactory:
    ...
    @staticmethod
    def create_wrapped_model(model: nn.Module, 
                           adapter: Optional[ModelAdapter] = None,
                           model_path: Optional[str] = None,
                           **kwargs) -> UnifiedTrainingWrapper:
        """
        ...
        if adapter is None:
            adapter = AutoAdapter()
            logging.info("No adapter specified, using AutoAdapter for automatic detection")
        ...
        logging.info(f"Created wrapper for {type(model).__name__} with {type(adapter).__name__}")
        return wrapper
    ...
    def from_config(config: Dict[str, Any]) -> UnifiedTrainingWrapper:
        ...
        # Other wrapper args
        wrapper_args = {k: v for k, v in config.items() 
                       if k not in ['model_class', 'model_args', 'adapter', 'adapter_config', 'model_path']}
    ...
    @staticmethod
    def _get_adapter(adapter_name: str, adapter_config: Dict[str, Any] = None) -> ModelAdapter:
        """Get adapter instance by name with optional configuration"""
        from ..adapters import (AutoAdapter, StandardAdapter, LogitsAndFeaturesAdapter, 
                               DictOutputAdapter, UCFAdapter, CapsuleNetworkAdapter,
        ...
        try:
            ...
            if registry_adapter:
                ...
                adapter_params = {k: v for k, v in adapter_config.items() 
        ...
        if adapter_name_lower == 'external':
            ...
            if 'script_path' not in adapter_config or 'class_name' not in adapter_config:
                raise ValueError("External adapter requires 'script_path' and 'class_name' in adapter_config")
        ...
        if adapter_name_lower in adapter_map:
            ...
            try:
                ...
                logging.warning(f"Adapter {adapter_name} doesn't accept configuration parameters")
...
class ModelUtils:
    ...
    def create_from_timm(model_name: str, **kwargs) -> UnifiedTrainingWrapper:
        ...
        try:
            ...
            raise ImportError("timm library not installed. Install with: pip install timm")
        
        model = timm.create_model(model_name, **kwargs)
        return ModelFactory.create_wrapped_model(model)
    
    @staticmethod
    def create_from_custom_path(model_file: str, class_name: str, **kwargs) -> UnifiedTrainingWrapper:
        """
    ...
    def validate_model_compatibility(model: nn.Module, 
                                   test_input_shape: tuple = (1, 3, 224, 224)) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if torch.is_tensor(output):
                ...
                output_info['output_shape'] = [item.shape if torch.is_tensor(item) else type(item).__name__ 
                                             for item in output]
                if len(output) == 2:
