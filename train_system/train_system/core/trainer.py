...
class UnifiedTrainer:
    ...
    def __init__(self, config: UnifiedTrainingConfig):
        ...
        self.non_blocking_transfer = getattr(
            self.config.training, "non_blocking_transfer", True
        )
        self.efficient_validation = getattr(
            self.config.training, "efficient_validation", True
        )
        ...
        try:
            if hasattr(self.config, "output") and hasattr(
                ...
                experiment_name = getattr(
                    self.config.output, "experiment_name", "experiment"
                )
            ...
            if (
                ...
                and hasattr(self.config.training, "tensorboard")
                and self.config.training.tensorboard
            ):
    ...
    def _setup_device(self) -> torch.device:
        ...
        if device.type == "cuda" and "memory_total_gb" in device_info:
            self.logger.info(
                f"GPU Memory: {device_info['memory_total_gb']:.1f}GB total, "
    ...
    def _initialize_registries(self):
        ...
        try:
            ...
            if registry_config and getattr(registry_config, "auto_scan", True):
                ...
                self.logger.debug(
                    "Auto-scan disabled, skipping registry initialization"
                )
        ...
        if enabled and name:
            try:
                ...
                if trainer_class:
                    ...
                    self.logger.warning(
                        f"External trainer '{name}' not found in registry"
                    )
            except ImportError:
    ...
    def log_epoch_results(self, epoch, train_loss, train_acc, val_loss, val_acc):
        ...
        print(
            f"Epoch {epoch_display:3d} | Train: {train_loss:.4f} ({train_acc:5.1f}%) | "
        ...
        self.log_file.write(
            f"{timestamp} | Epoch {epoch_display:3d} | "
    ...
    def load_model(self):
        ...
        if hasattr(model_config, "external_adapter") and model_config.external_adapter:
            ...
            try:
                ...
                logging.info(
                    f"Using external adapter: {model_config.external_adapter.get('class_name')}"
                )
            except Exception as e:
                logging.error(
                    f"Failed to load external adapter: {e}, falling back to default"
                )

        # If external adapter wasn't loaded, check for standard adapter config
        if (
            adapter is None
            and hasattr(model_config, "adapter")
            and model_config.adapter
        ):
        ...
        self.wrapper = ModelFactory.create_wrapped_model(
            self.model, adapter=adapter, num_classes=model_config.num_classes
        )
        ...
        trainable_params = sum(
            p.numel() for p in self.wrapper.parameters() if p.requires_grad
        )
            ...
            if isinstance(self.model.classifier, nn.Linear):
                ...
                for i, layer in enumerate(reversed(self.model.classifier)):
                    if isinstance(layer, nn.Linear):
                        ...
                        self.model.classifier[-1 - i] = nn.Linear(
                            in_features, model_config.num_classes
                        )
                        break
        ...
        try:
            ...
            if model_config.class_name:
                ...
                if not hasattr(module, model_config.class_name):
                    self.logger.error(
                        f"❌ Class {model_config.class_name} not found in {model_path}"
                    )
                    available_classes = [
                ...
                model_classes = [
                    ...
                    and getattr(module, name) != nn.Module
                ]
                ...
                if len(model_classes) == 0:
                    ...
                    raise ValueError(
                        f"Multiple model classes found: {class_names}. Please specify class_name"
                    )
                else:
                    model_class = model_classes[0]
                    self.logger.info(
                        f"Auto-detected model class: {model_class.__name__}"
                    )
        ...
        try:
            ...
            if isinstance(checkpoint, dict):
                if "model_state_dict" in checkpoint:
                    ...
                    if (
                        model_config.load_optimizer
                        and "optimizer_state_dict" in checkpoint
                    ):
                        self._stored_optimizer_state = checkpoint[
                            "optimizer_state_dict"
                        ]
                        self.logger.info("💾 Optimizer state will be restored")

                    if (
                        model_config.load_scheduler
                        and "scheduler_state_dict" in checkpoint
                    ):
                        self._stored_scheduler_state = checkpoint[
            ...
            if model_config.strict_loading:
                missing_keys, unexpected_keys = self.wrapper.load_state_dict(
                    state_dict, strict=True
                )
                self.logger.info(
                    "✅ Pretrained weights loaded successfully (strict mode)"
                )
            else:
                missing_keys, unexpected_keys = self.wrapper.load_state_dict(
                ...
                if missing_keys:
                    self.logger.warning(
                        f"⚠️ Missing keys in pretrained weights: {len(missing_keys)} keys"
                    )
                    self.logger.debug(f"Missing keys: {missing_keys}")

                if unexpected_keys:
                    self.logger.warning(
                        f"⚠️ Unexpected keys in pretrained weights: {len(unexpected_keys)} keys"
                    )
                    self.logger.debug(f"Unexpected keys: {unexpected_keys}")

                self.logger.info(
                    "✅ Pretrained weights loaded successfully (non-strict mode)"
                )
    ...
    def setup_optimizer(self):
        ...
        if (
            hasattr(self.config.model, "num_classes")
            and self.config.model.num_classes > 1
        ):
            self.criterion = torch.nn.CrossEntropyLoss()
            self.logger.info(
                f"✅ Loss function: CrossEntropyLoss for {self.config.model.num_classes} classes"
            )
        else:
            # Binary classification or regression
            if (
                hasattr(self.config.data, "class_mapping")
                and len(self.config.data.class_mapping) == 2
            ):
                self.criterion = torch.nn.CrossEntropyLoss()
                self.logger.info(
                    f"✅ Loss function: CrossEntropyLoss for binary classification"
                )
            else:
        ...
        if training_config.scheduler.lower() == "cosine":
            ...
            scheduler_params = {}
            if (
                hasattr(training_config, "scheduler_params")
                and training_config.scheduler_params
            ):
            ...
            eta_min = scheduler_params.pop(
                "eta_min", training_config.learning_rate * 0.01
            )

            print(
                f"🔧 DEBUG: T_max = {T_max}, eta_min = {eta_min}, remaining params = {scheduler_params}"
            )

            self.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
                self.optimizer, T_max=T_max, eta_min=eta_min, **scheduler_params
            )
        ...
        if (
            hasattr(self, "_stored_scheduler_state")
            and self._stored_scheduler_state
            and self.scheduler
        ):
            try:
    ...
    def train_epoch(self) -> Dict[str, float]:
        ...
        for batch_idx, (images, labels) in enumerate(self.train_loader):
            ...
            compute_detailed_metrics = (
                self.metrics_frequency == 0  # Always compute if set to 0
                or batch_idx % max(self.metrics_frequency, 1) == 0
                or batch_idx
                == len(self.train_loader) - 1  # Always compute for last batch
            )
            ...
            if compute_detailed_metrics:
                ...
                if self.writer and batch_idx % self.config.training.log_interval == 0:
                    ...
                    self.writer.add_scalar(
                        "Accuracy/Train_Batch", current_accuracy, global_step
                    )
            else:
    ...
    def get_weight_extension(self) -> str:
        ...
        if self.config.output.weight_format == "auto":
            ...
            self.logger.warning(
                f"Unknown weight format: {self.config.output.weight_format}, using .pth"
            )
            return "pth"
    ...
    def save_checkpoint(self, epoch: int, is_best: bool = False):
        ...
        if keep_recent > 0:
            checkpoint_data["train_losses"] = (
                self.train_losses[-100:]
                if len(self.train_losses) > 100
                else self.train_losses
            )
            checkpoint_data["val_accuracies"] = (
                self.val_accuracies[-20:]
                if len(self.val_accuracies) > 20
                else self.val_accuracies
            )
        else:
    ...
    def _load_checkpoint_and_resume(self, checkpoint_path: Path) -> int:
        ...
        try:
            ...
            completed_epoch = checkpoint.get(
                "epoch", 0
            )  # This is 1-based (human readable)
            start_epoch = completed_epoch  # Use completed epoch as 0-based loop index

            self.logger.info(
                f"✅ Checkpoint was saved after completing epoch {completed_epoch}"
            )
            self.logger.info(
                f"✅ Resuming training from epoch {start_epoch + 1} (will be displayed as epoch {start_epoch + 1})"
            )
    ...
    def _train_epoch_with_progress(self, epoch):
        ...
        for data, target in pbar:
            ...
            pbar.set_postfix(
                {"Loss": f"{loss.item():.4f}", "Acc": f"{current_acc:.1f}%"}
            )
