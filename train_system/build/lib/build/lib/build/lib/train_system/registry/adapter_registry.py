...
class AdapterRegistry:
    ...
    def scan_for_adapters(self, additional_paths: List[str] = None):
        ...
        logger.info(f"Found {len(self._adapters)} adapters: {list(self._adapters.keys())}")
    ...
    def _is_adapter_class(self, cls) -> bool:
        ...
        has_required = all(hasattr(cls, method) and callable(getattr(cls, method)) 
                          for method in required_methods)
        has_optional = any(hasattr(cls, method) and callable(getattr(cls, method)) 
    ...
    def register_adapter(self, name: str, adapter_class: Type, force: bool = False):
        ...
        if name in self._adapters and not force:
            logger.warning(f"Adapter '{name}' already registered. Use force=True to override.")
            return
