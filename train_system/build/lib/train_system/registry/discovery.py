...
def initialize_registries(config=None, force_rescan: bool = False, additional_paths: dict = None):
    ...
    try:
        ...
        logger.info(f"Found {adapter_count} adapters: {list(discovered_components.get('adapters', {}).keys())}")
        logger.info(f"Found {trainer_count} trainers: {list(discovered_components.get('trainers', {}).keys())}")
...
def _discover_components_dynamically(search_paths: List[str]) -> Dict[str, Dict[str, Any]]:
