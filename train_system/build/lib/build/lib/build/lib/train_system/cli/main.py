...
def train_from_config(config_path: str):
    ...
    try:
        ...
        # Handle different result key formats from different trainers
        accuracy_keys = ['best_val_accuracy', 'final_accuracy', 'best_accuracy', 'accuracy']
        accuracy_value = None
        
        for key in accuracy_keys:
            if key in results:
                accuracy_value = results[key]
                break
        
        if accuracy_value is not None:
            print(f"🏆 Best validation accuracy: {accuracy_value:.2f}%")
        else:
            print("🏆 Training completed (accuracy not available)")
            if results:
