...
class ModelConfig:
    ...
    pretrained_weights: Optional[str] = (
        None  # Path to pretrained weights (.pth, .pt file)
    )
    ...
    adapter: Optional[str] = (
        None  # "auto", "standard", "logits_features", "dict", "external", etc.
    )
    ...
    external_adapter: Optional[Dict[str, Any]] = (
        None  # {"script_path": "path/to/adapter.py", "class_name": "MyAdapter"}
    )
...
class DataConfig:
    ...
    class_folders: Dict[str, str] = field(
        default_factory=dict
    )  # {"class_name": "path"}
    ...
    class_mapping: Dict[str, int] = field(
        default_factory=lambda: {"real": 0, "fake": 1}
    )

    # Automatic train/val splitting (when val_path is None)
    auto_split_enabled: bool = (
        True  # Enable automatic train/val split when val_path is None
    )
    train_split_ratio: float = (
        0.8  # Proportion of data used for training (0.8 = 80% train, 20% val)
    )
    split_seed: Optional[int] = (
        42  # Random seed for reproducible splits (None for random)
    )
...
class TrainingConfig:
    ...
    metrics_frequency: int = (
        100  # Calculate detailed metrics every N steps (0 = every step)
    )
    ...
    resume_from_checkpoint: Optional[Union[str, bool]] = (
        None  # Path to checkpoint or True for auto-find
    )
...
class OutputConfig:
    ...
    keep_recent_checkpoints: int = (
        3  # Only keep N recent full checkpoints (0 = keep all)
    )
...
class ExternalTrainerConfig:
    ...
    skip_unified_data_loading: bool = (
        False  # Skip UnifiedDataset creation (for trainers that handle their own data)
    )
...
class UnifiedTrainingConfig:
    ...
    external_trainer: Optional[ExternalTrainerConfig] = field(
        default_factory=lambda: ExternalTrainerConfig()
    )
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "external_trainer": (
                self.external_trainer.__dict__ if self.external_trainer else {}
            ),
            "device": self.device,
            "seed": self.seed,
    ...
    def validate(config: UnifiedTrainingConfig) -> "ValidationResult":
        ...
        if config.model.type == "file":
            ...
            if not config.model.class_name:
                warnings.append(
                    "Model class_name not specified, will try to auto-detect"
                )
...
class ConfigTemplateManager:
    ...
    BLIP_CONFIG_TEMPLATE = {
        "model": {
            ...
            "model_args": {"vit_arch": "base", "drop_path_rate": 0.1},
        },
        "data": {
            "name": "URS",
            "type": "folder",
    ...
    GENERIC_CONFIG_TEMPLATE = {
        ...
        "output": {"output_dir": "training_output", "experiment_name": "my_experiment"},
    }
    ...
    def create_config_from_template(
        cls, template_name: str, **overrides
    ...
    @classmethod
    def save_template(
        cls, template_name: str, output_path: Union[str, Path], format: str = "yaml"
    ):
