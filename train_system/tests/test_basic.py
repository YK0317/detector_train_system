...
def test_adapters():
    ...
    assert torch.equal(
        logits, tuple_output[0]
    ), "AutoAdapter should extract first element"
    ...
    ), "AutoAdapter should extract logits key"
...
def test_configuration():
    ...
    print(
        f"✅ Configuration validation completed (errors: {len(validation_result.errors)})"
    )
...
def test_model_utils():
    ...
    print(
        f"✅ Model compatibility check passed: {compatibility['recommended_adapter']}"
    )
