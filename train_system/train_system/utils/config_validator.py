...
class ConfigValidator:
    ...
    def validate_and_fix(
        self, config_dict: Dict[str, Any], config_path: Optional[Path] = None
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if device_str != new_device_str:
                self.auto_fixes.append(
                    f"Changed device from '{device_str}' to '{new_device_str}' (availability check)"
                )
    ...
    def _fix_data_config(self, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if data_config.get("type") == "class_folders":
            if "class_mapping" not in data_config:
                ...
                if "real_path" in data_config and "fake_path" in data_config:
                    ...
                    self.auto_fixes.append(
                        "Generated class_folders from real_path/fake_path"
                    )

                if class_folders:
                    data_config["class_mapping"] = {
                        name: idx for idx, name in enumerate(class_folders.keys())
                    }
                    self.auto_fixes.append(
                        "Auto-generated class mapping from class folders"
                    )
        ...
        if device_str == "cpu" and batch_size > 16:
            ...
            self.auto_fixes.append(
                f"Reduced batch_size from {batch_size} to 16 for CPU compatibility"
            )
        ...
        if platform.system() == "Windows":
            ...
            if num_workers > 2:
                ...
                self.auto_fixes.append(
                    f"Reduced num_workers from {num_workers} to 2 for Windows stability"
                )
    ...
    def _fix_training_config(self, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if scheduler == "cosine":
            ...
            if "T_max" not in scheduler_params:
                ...
                scheduler_params["T_max"] = epochs
                self.auto_fixes.append(
                    f"Updated T_max to match epochs ({epochs}) for cosine scheduler"
                )
    ...
    def _apply_smart_defaults(self, config_dict: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, default_value in data_defaults.items():
            if key not in data_config:
                ...
                self.auto_fixes.append(
                    f"Applied smart default: data.{key} = {default_value}"
                )
        ...
        for key, default_value in training_defaults.items():
            if key not in training_config:
                ...
                self.auto_fixes.append(
                    f"Applied smart default: training.{key} = {default_value}"
                )
        ...
        if "resume_from_checkpoint" in training_config:
            ...
            if checkpoint_path:
                ...
                if resolved_path and resolved_path.exists():
                    ...
                    self.auto_fixes.append(
                        f"Resolved checkpoint path: {checkpoint_path}"
                    )
                else:
        ...
        output_defaults = {"save_best_only": False, "save_lightweight": True}

        for key, default_value in output_defaults.items():
            if key not in output_config:
                output_config[key] = default_value
                self.auto_fixes.append(
                    f"Applied smart default: output.{key} = {default_value}"
                )
